...
var require_index_688c5d50 = __commonJS({
  "node_modules/degit/dist/index-688c5d50.js"(exports2) {
    ...
    var replace = (opt, files, cb) => {
      ...
      const promise = new Promise((resolve2, reject) => {
        ...
        const onopen = (er, fd) => {
          ...
          if (er)
            return reject(er);
          fs__default["default"].fstat(fd, (er2, st) => {
            if (er2)
              return reject(er2);
    ...
    var pathReservations = () => {
      ...
      const { join: join5 } = path__default["default"];
      const getDirs = (path2) => join5(path2).split(/[\\/]/).slice(0, -1).reduce((set, path3) => set.length ? set.concat(join5(set[set.length - 1], path3)) : [path3], []);
    ...
    var Unpack = class extends parse$4 {
      ...
      [CHECKFS2](entry, done) {
        this[MKDIR](path__default["default"].dirname(entry.absolute), this.dmode, (er) => {
          ...
          fs__default["default"].lstat(entry.absolute, (er2, st) => {
    ...
    function rimraf_(p, options2, cb) {
      ...
      assert__default["default"](typeof cb === "function");
      options2.lstat(p, function(er, st) {
        if (er && er.code === "ENOENT")
          return cb(null);
      ...
      constructor(src2, opts = {}) {
        ...
        this.directiveActions = {
          clone: async (dir, dest, action) => {
            ...
            const d2 = degit(action.src, opts2);
            d2.on("info", (event) => {
              console.error(
      ...
      async clone(dest) {
        ...
        if (directives) {
          for (const d2 of directives) {
            await this.directiveActions[d2.action](dir, dest, d2);
          }
        ...
        if (removedFiles.length > 0) {
          this._info({
            ...
            message: `removed: ${source.bold(
              removedFiles.map((d2) => source.bold(d2)).join(", ")
            )}`
          });
        }
      }
...
async function removeCommand(name, options2) {
  const spinner = (0, import_ora2.default)(`Removing agent "${name}"...`).start();
  try {
    let platforms;
    if (options2.agent) {
      platforms = [options2.agent];
